...
import {
  ...
  SlidersHorizontal, Star, Trash2, Twitter, X,
} from 'lucide-react';
...
function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!isReady) return;
    setCart((current) => { const kept = current.filter((item) => findProduct(item.id)); return kept.length === current.length ? current : kept; });
  ...
  const value = useMemo<StoreContextValue>(() => ({
    ...
    addToCart: (id, quantity = 1, variant = 'Default') => {
      setCart((current) => {
        ...
        return existing ? current.map((item) => item === existing ? { ...item, quantity: item.quantity + quantity } : item) : [...current, { id, quantity, variant }];
      });
      setToast(`${findProduct(id)?.title || 'Item'} added to your bag`);
    },
    updateQuantity: (id, quantity, variant) => setCart((current) => quantity < 1 ? current.filter((item) => item.id !== id || item.variant !== variant) : current.map((item) => item.id === id && item.variant === variant ? { ...item, quantity } : item)),
...
function Header() {
  ...
  const submitSearch = (event: FormEvent) => { event.preventDefault(); if (query.trim()) { setLocation(`/search?q=${encodeURIComponent(query.trim())}`); setMenuOpen(false); } };
    ...
    <div className="container-lunaria flex h-[74px] items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-1.5">
        <form onSubmit={submitSearch} className="hidden items-center rounded-full border border-[#e6d4d7] bg-[#FFF1EC] px-3 py-2 sm:flex" role="search">
          <Search size={16} className="mr-2 text-[#8B659C]" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-[130px] bg-transparent text-sm outline-none placeholder:text-[#8b7c87]" placeholder="Search the shelves" aria-label="Search the shelves" data-testid="input-header-search" />
        </form>
...
function Footer() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (email.includes('@')) setSuccess(true); };
...
function ProductCard({ product, index = 0 }: { product: Product; index?: number }) {
  ...
  return <article className="group animate-rise" style={{ animationDelay: `${index * 70}ms` }} data-testid={`card-product-${product.id}`}>
    ...
    <div className="pt-4"><div className="flex items-start justify-between gap-3"><div><Link href={`/product/${product.id}`} className="font-display text-[1.18rem] leading-5 text-[#30263B] hover:text-[#48458F]" data-testid={`link-title-${product.id}`}>{product.title}</Link><p className="mt-1 text-xs text-[#746875]">{product.author || product.type}</p></div><p className="whitespace-nowrap text-sm font-semibold text-[#48458F]">{money(product.price)}</p></div><div className="mt-3 flex items-center justify-between"><span className="flex items-center gap-1 text-xs text-[#746875]"><Star size={13} fill="#B274A2" className="text-[#B274A2]" /> {product.rating}</span><button onClick={() => addToCart(product.id)} className="text-xs font-bold uppercase tracking-[.13em] text-[#48458F] underline decoration-[#F8B2B2] decoration-2 underline-offset-4 hover:text-[#B274A2]" data-testid={`button-add-${product.id}`}>Add to bag</button></div></div>
  </article>;
}
...
function Catalog({ kind, title, eyebrow, description }: { kind?: ProductKind; title: string; eyebrow: string; description: string }) {
  ...
  const filtered = useMemo(() => products.filter((product) => !kind || product.kind === kind).filter((product) => filter === 'all' || product.availability.startsWith('Only')).sort((a, b) => sort === 'low' ? a.price - b.price : sort === 'high' ? b.price - a.price : sort === 'rating' ? b.rating - a.rating : products.indexOf(a) - products.indexOf(b)), [products, kind, sort, filter]);
...
function SearchPage() {
  ...
  const { products, isLoading, isError } = useCatalog();
  const results = products.filter((product) => `${product.title} ${product.author || ''} ${product.type || ''}`.toLowerCase().includes(query.toLowerCase())).slice(0, 8);
  ...
  return <Shell><div className="container-lunaria py-14 md:py-20"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#B274A2]">Take a look around</p><h1 className="mt-3 font-display text-5xl text-[#30263B] md:text-6xl">Search the shelves</h1><form onSubmit={submit} className="mt-8 flex max-w-2xl items-center border-b-2 border-[#48458F] py-3"><Search size={20} className="mr-3 text-[#B274A2]" /><input autoFocus value={value} onChange={(event) => setValue(event.target.value)} className="min-w-0 flex-1 bg-transparent text-lg outline-none placeholder:text-[#b2a4ab]" placeholder="Try “poetry”, “journal”, or an author" aria-label="Search catalog" data-testid="input-search-page" /><button className="text-sm font-bold text-[#48458F]" data-testid="button-search-submit">Search</button></form>{query && <p className="mt-10 text-sm text-[#746875]">Showing {results.length} results for <strong className="text-[#30263B]">“{query}”</strong></p>}{query && results.length > 0 && <div className="mt-8 grid grid-cols-2 gap-x-4 gap-y-12 md:grid-cols-3 md:gap-7 lg:grid-cols-4">{results.map((product, index) => <ProductCard key={product.id} product={product} index={index} />)}</div>}{query && results.length === 0 && <EmptyState title="No exact match" message="Try a shorter phrase, or browse the shelves by mood instead." href="/shop" label="See all products" />}</div></Shell>;
}
