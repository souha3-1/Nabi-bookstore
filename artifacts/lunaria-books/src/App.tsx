...
function Header() {
  ...
  return <header className="sticky top-0 z-40 border-b border-[#eadbd9] bg-[#FFF9F7]/95 backdrop-blur-md">
    ...
    {menuOpen && <div className="border-t border-[#eadbd9] bg-[#FFF9F7] px-4 py-5 lg:hidden animate-rise"><nav className="container-lunaria flex flex-col gap-4" aria-label="Mobile navigation">{navItems.map(([label, href]) => <Link key={href} href={href} onClick={() => setMenuOpen(false)} className="font-display text-2xl text-[#30263B]" data-testid={`link-mobile-${label.toLowerCase().replace(' ', '-')}`}>{label}</Link>)}<form onSubmit={submitSearch} className="mt-2 flex items-center border-b border-[#8B659C] py-2"><button type="submit" aria-label="Search" className="mr-2 text-[#8B659C]"><Search size={17} /></button><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent outline-none" placeholder="Find a title, author, or color" aria-label="Search" data-testid="input-mobile-search" /></form></nav></div>}
  </header>;
}
...
function Footer() {
  return <footer className="mt-24 bg-[#30263B] text-[#FFF9F7]">
    <div className="container-lunaria grid gap-12 py-14 md:grid-cols-[1.4fr_1fr]">
     ...
     <div className="border-t border-[#4e435b] py-5 text-center text-xs text-[#aa9eae]">© 2026 NABI NOVELS · Made by <a href="https://www.instagram.com/tired.sou?stkn=azZwcWowa3A3NjJ5" target="_blank" rel="noopener noreferrer" className="underline hover:text-white" data-testid="link-credit">Souha Nekamiche</a>.</div>
  </footer>;
}
